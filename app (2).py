...
if bg_path:
    ...
    # Using safe template concatenation to prevent Python f-string bracket compilation errors
    custom_style = '<style>'
    custom_style += '.stApp, [data-testid="stHeader"], [data-testid="stSidebar"], [data-testid="stAppViewContainer"] { background-color: #FFFFFF !important; }'
    custom_style += 'input, textarea, select, div[data-baseweb="input"], div[data-baseweb="select"] { background-color: #F8FAFC !important; color: #1E293B !important; border: 1px solid #CBD5E1 !important; -webkit-text-fill-color: #1E293B !important; }'
    custom_style += 'input::placeholder, textarea::placeholder, .stTextInput input::placeholder { color: #94A3B8 !important; opacity: 1 !important; -webkit-text-fill-color: #94A3B8 !important; }'
    custom_style += '.stApp::before { content: ""; position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; z-index: -1; background-image: url("data:image/png;base64,' + encoded_logo + '"); background-repeat: no-repeat; background-position: center center; background-size: 35%; opacity: 0.08; pointer-events: none; }'
    custom_style += 'h1, h2, h3, h4, h5, h6, p, label, .stMarkdown, div[data-testid="stMarkdownContainer"] p, span, .stSelectbox label { color: #1E293B !important; }'
    custom_style += '</style>'
