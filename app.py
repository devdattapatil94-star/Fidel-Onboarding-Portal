...
if st.button("Submit Profile Record", type="primary"):
    ...
    if has_first_name and has_email and has_native_lang:
        ...
        body_text = f"🚨 NEW VENDOR REGISTRATION DATA%0A%0A"                     f"Full Name: {f_name.strip()} {l_name.strip()}%0A"                     f"Email ID: {v_email.strip()}%0A"                     f"Phone: {v_phone.strip()}%0A"                     f"Availability: {avail}%0A"                     f"Native Language: {native.strip()}%0A%0A"                     f"🏦 BANK WIRE VAULT:%0A"                     f"- Bank: {b_name}%0A"                     f"- Holder: {b_holder}%0A"                     f"- Code/Acc: {b_code} / {b_acc}%0A"                     f"- IFSC/Swift: {b_ifsc} / {b_swift}%0A"                     f"- Bank Address: {b_address.replace('%0A', ' ')}%0A%0A"                     f"⚖️ TAX REGISTRATIONS:%0A"                     f"- PAN Card: {b_tax}%0A"                     f"- GST Number: {b_gst}%0A%0A"                     f"💳 DIGITAL ECOSYSTEMS:%0A"                     f"- PayPal ID: {pay_paypal}%0A"                     f"- Payoneer: {pay_payoneer}%0A"                     f"- ProZ*Pay Link: {pay_proz}%0A%0A"                     f"📦 UPLOAD SUMMARY:%0A"                     f"- NDA: {'Yes' if file_nda else 'No'}, PO: {'Yes' if file_po else 'No'}%0A"                     f"- Certs/Ref: {'Yes' if file_cert else 'No'}/{'Yes' if file_edu else 'No'}/{'Yes' if file_ref else 'No'}"
