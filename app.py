...
with col2:
    ...
    # Country Code Selection alongside local phone number input
    ...
    with c_code_col:
        country_codes = [
            "+91 (IN)", "+1 (US/CA)", "+44 (UK)", "+81 (JP)", "+49 (DE)", "+33 (FR)", 
            "+61 (AU)", "+86 (CN)", "+7 (RU)", "+39 (IT)", "+34 (ES)", "+971 (AE)", 
            "+65 (SG)", "+55 (BR)", "+27 (ZA)", "+82 (KR)", "+41 (CH)", "+31 (NL)"
        ]
        selected_code = st.selectbox("Code", country_codes, label_visibility="collapsed")
    with phone_num_col:
        v_phone_local = st.text_input("Phone Number", placeholder="XXXXX-XXXXX", label_visibility="collapsed")
    
    # Combine code and local number for validation logic
...
if st.button("Submit Profile Record", type="primary"):
    ...
    if (v_first_name and v_last_name and v_email_id and v_contact and v_city and 
        ...
        st.balloons()
        st.success("🎉 Onboarding Profile Verification Clear!")
        
        body_text = f"🚨 NEW VENDOR REGISTRATION DATA%0A%0A"                     f"Full Name: {f_name.strip()} {l_name.strip()}%0A"                     f"Email ID: {v_email.strip()}%0A"                     f"Phone: {v_phone}%0A"                     f"Availability: {avail}%0A"                     f"Native Language: {native.strip()}%0A%0A"                     f"🏦 BANK INFO:%0A"                     f"- Bank: {b_name}%0A"                     f"- Holder: {b_holder}%0A"                     f"- Code/Acc: {b_code} / {b_acc}%0A"                     f"- IFSC/Swift: {b_ifsc} / {b_swift}%0A%0A"                     f"⚖️ TAX REGISTRATIONS:%0A"                     f"- PAN Card: {b_tax}%0A"                     f"- GST Number: {b_gst}%0A%0A"                     f"💳 DIGITAL ECOSYSTEMS:%0A"                     f"- PayPal ID: {pay_paypal}%0A"                     f"- Payoneer: {pay_payoneer}%0A"                     f"- ProZ*Pay Link: {pay_proz}%0A%0A"                     f"📦 UPLOAD SUMMARY:%0A"                     f"- NDA: Yes, PO: Yes, Consent Form: Yes%0A"                     f"- Edu Certificates: Yes, Reference Letter: Yes%0A"                     f"- Translation Cert: {'Yes' if file_cert else 'No'}"
        ...
        if not v_compliance: missing_fields.append("Compliance Documentation Submission (All 3 signed templates)")
        ...
        st.error(f"❌ Submission Failed. The following mandatory requirements are missing: {', '.join(missing_fields)}")
