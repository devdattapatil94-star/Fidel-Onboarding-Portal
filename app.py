...
# Full Address Fields from Word File
...
# Comprehensive Multi-select options from Word File
cat_options = [
...
if st.button("Submit Profile Record", type="primary"):
    ...
    if has_first_name and has_email and has_native_lang:
        ...
        submission_summary = {
            ...
            "Translation Cert Provided": "Yes" if file_cert else "No",
            "Edu Certificates Provided": "Yes" if file_edu else "No",
            "Reference Letter Provided": "Yes" if file_ref else "No"
        }
        st.json(submission_summary)
        
        body_text = f"🚨 NEW VENDOR REGISTRATION DATA%0A%0A"                     f"Full Name: {f_name.strip()} {l_name.strip()}%0A"                     f"Email ID: {v_email.strip()}%0A"                     f"Phone: {v_phone.strip()}%0A"                     f"Address: {addr_street}, {addr_city}, {addr_state}, {addr_country}%0A"                     f"Availability: {avail}%0A"                     f"Native Language: {native.strip()}%0A"                     f"Language Pairs: {lang_pairs.strip()}%0A"                     f"CAT Tools: {', '.join(selected_cat_tools)}%0A"                     f"Domains: {', '.join(selected_domains[:5])}...%0A"                     f"Services: {', '.join(selected_services[:5])}...%0A"                     f"Bank: {b_name} | Acc: {b_acc} | IFSC: {b_ifsc}%0A"                     f"PAN Card Number: {b_tax.strip()}%0A%0A"                     f"📦 UPLOAD STATUS:%0A"                     f"- NDA Uploaded: {'Yes' if file_nda else 'No'}%0A"                     f"- PO Terms Signed: {'Yes' if file_po else 'No'}%0A"                     f"- Consent Form Uploaded: {'Yes' if file_consent else 'No'}%0A"                     f"- Translation Cert: {'Yes' if file_cert else 'No'}%0A"                     f"- Edu Certificates: {'Yes' if file_edu else 'No'}%0A"                     f"- Reference Letter: {'Yes' if file_ref else 'No'}"
