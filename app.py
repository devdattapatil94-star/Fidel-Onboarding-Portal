...
"Arabic (Syria)", "Arabic (Tunisia)", "Arabic (United Arab Emirates)", "Arabic (Western 
Sahara)", 
...
"Basque", "Bassa (BSQ)", "Bemba", "Bengali", "Bengali (Bangladesh)", "Berber 
(Tamazight)", 
"Bhili (India)", "Bhojpuri", "Bhutani", "Bihari", "Bikol", "Bislama", "Bodo", "Bosnian 
(Cyrillic)", 
"Bosnian (Latin)", "Breton", "Bulgarian", "Buli", "Bundeli", "Burmese", "Burushaski", 
"Burushaski (Pakistan)", "Byelorussian", "Cantonese", "Carolinian", "Catalan", "Catalan 
(Spain)", 
"Cebuano (Philippines)", "Cham", "Chamorro", "Chhattisgarhi (India)", "Chichewa", "Chin 
(Falam)", 
"Chinese (Mandarin)", "Chinese (Simplified)", "Chinese (Singapore)", "Chinese 
(Traditional)", 
"Chinese, Hong Kong", "Chittagonian (CTG)", "Chuukese", "Corsican", "Croatian", "Czech", 
"Dandami Maria (India)", "Danish", "Dari", "Dhivehi-Maldivian (Maldives)", "Dholuo", 
"Dhundari", 
...
"English(Malaysia)", "English (Asia)", "English (Austria)", "English (Cyprus)", "English 
(Germany)", 
...
"French (Cameroon) (FR CM)", "French (Canada)", "French (France)", "French (Haiti) (FR
Haiti)", 
"French (Luxembourg)", "French (Morocco)", "French (Switzerland)", "Frisian", "Fula", 
"Galician (Spain)", "Garhwali", "Garo", "Georgian", "German (Austria)", "German 
(Belgium)", 
"German (Germany)", "German (Holland)", "German (Luxembourg)", "German 
(Switzerland)", 
"Gondi", "Greek", "Greenlandic", "Guarani", "Gujarati (India)", "Haitian Creole", "Hakha 
Chin", 
...
"Kazakh", "Khashi", "Khasi", "Khmer", "Khmu (KJG)", "Khowar", "Kikongo (Congo)", 
"Kikuyu", 
"Kinyarwanda", "Kirghiz", "Kiribati", "Kirundi (Rundi) (RN)", "Kiswahili (Africa)", 
"Klingon", "Konkani (India)", "Korean", "Kosraean", "Kru", "Kuanyama", "Kurdish 
(Turkey)", 
...
"Maasai (MAS)", "Maay", "Macedonian", "Macedonian (MK)", "Maithili", "Malagasy 
(Madagascar)", 
"Malay (Malysia)", "Malay Chinese", "Malayalam (India)", "Malinke", "Maltese", 
"Manipuri", 
"Maori", "Marathi", "Maria-Dandami", "Marshallese", "Marwari", "Masalit", "Meru 
(Kimeru) (MER)", 
"Mewari", "Mien (Iu Mien) (IUM)", "Mirpuri", "Mizo", "Moldavian", "Mongolian", 
"Montenegrin", 
"Morisyen (Mauritian Créole)", "Nagamese", "Nankam", "Nauru", "Navajo", "Ndebele", 
"Nepali", 
"Niuean", "Nkore", "Nobiin", "Northern Sotho (South Africa)", "Norwegian", "Norwegian 
(Bokmål)", 
"Norwegian Bokmaal (Norway)", "Norwegian Nynorsk (Norway)", "Nuer", "Nyanja", 
"Nyoro", 
"Occitan", "Oriya", "Oromo", "Ossetian", "Ottoman Turkish", "Pahari", "Pak Thai (Dambro) 
(SOU)", 
"Palauan", "Pampanga (Kapampangan)", "Pangasinan", "Papiamento", "Pashto", 
"Persian", 
...
"Punjabi (Pakistan)", "Quechua", "Quenya", "Ratotongan", "Rhaeto-Romance", 
"Rohingya", 
...
"Seselwa Creole French", "Sesotho", "Setswana (Africa)", "Shan", "Sherpa (Nepal)", 
"Shona", 
"Sindarin", "Sindhi", "Singhalese", "Siswati", "Slovak", "Slovene", "Slovenian (SL)", 
"Somali", "Sorbian (Lower)", "Sorbian (Upper)", "Sotho", "Spanish (Argentina)", "Spanish 
(Bolivia)", 
"Spanish (Chile)", "Spanish (Colombia)", "Spanish (Costa Rica)", "Spanish (Cuba)", 
"Spanish (Dominican Republic)", "Spanish (Ecuador)", "Spanish (El Salvador)", "Spanish 
(Guatemala)", 
"Spanish (Honduras)", "Spanish (Latin America)", "Spanish (Mexico)", "Spanish 
(Nicaragua)", 
"Spanish (Panama)", "Spanish (Paraguay)", "Spanish (Spain)", "Spanish (USA)", 
"Sundanese (SU)", 
"Swahili (Burundi)", "Swahili (Kenya)", "Swahili (Rwanda)", "Swahili (Tanzania)", "Swahili 
(Uganda)", 
...
"Tok Pisin", "Tonga (Polynesian)", "Tongan", "Tulu", "Turkish", "Turkmen (TK)", "Twi 
(TW)", 
"Ukrainian", "Upper Guinea Creole", "Urdu", "Urdu (India)", "Uyghur", "Uzbek (UZ)", 
"Venda (VE)", "Vietnamese (VI)", "Waray-Waray (WW)", "Xhosa (XH)", "Xârâcùù (New 
Caledonia)", 
...
st.warning("⚠️ Place FIDEL.NSE.png in your repo") with 
col_title: 
st.markdown("<h1 style='margin: 0; padding: 0; font-size: 2.25rem; white-space: 
nowrap;'>Fidel Softech Resource Onboarding</h1>", unsafe_allow_html=True) 
st.markdown("Please complete the official empanelment profile form below, complete the 
translation test task, and upload your paperwork.") 
st.markdown("---") 
...
st.markdown("<label style='font-size: 14px;'>Contact Number *</label>", 
unsafe_allow_html=True) 
c_code_col, phone_num_col = st.columns([1, 2]) 
with c_code_col: 
all_country_codes = ["+91 (India)", "+1 (USA/Canada)", "+44 (UK)", "+81 (Japan)", "+49 
(Germany)", "+33 (France)", "+61 (Australia)"] 
selected_code = st.selectbox("Code", all_country_codes, label_visibility="collapsed") 
with phone_num_col: 
v_phone_local = st.text_input("Phone Number", placeholder="XXXXX-XXXXX", 
label_visibility="collapsed") 
v_phone = f"{selected_code.split(' ')[0]} {v_phone_local.strip()}" if v_phone_local.strip() 
else "" 
...
selected_cat_tools = st.multiselect("Proficient in which of the following CAT Tools:", 
cat_options) 
domain_options = [ 
...
with rate_curr_col: 
rate_currency = st.selectbox("Preferred Currency *", ["USD ($)", "JPY (¥)", "INR (₹)", "EUR 
(€)", "GBP (£)"]) 
with rate_trans_col: 
rate_per_word = st.number_input("Translation Rate (per word) *", min_value=0.000, 
step=0.001, format="%.3f") 
with rate_edit_col: 
rate_per_hour = st.number_input("Editing/Review Rate (per hour) *", min_value=0.00, 
step=0.50, format="%.2f") 
...
with d_col1: 
st.download_button("📥 Download NDA Template", data=nda_data, 
file_name="Fidel_NDA_Ver 1.3.pdf", mime="application/pdf", disabled=(len(nda_data) == 
0)) 
with d_col2: 
st.download_button("📥 Download PO Terms", data=po_data, file_name="Fidel_PO
Invoice-Payment-Procedure_ver_1.3.pdf", mime="application/pdf", disabled=(len(po_data) 
== 0)) 
with d_col3: 
st.download_button("📥 Download Consent Form", data=consent_data, 
file_name="Fidel Consent Form.pdf", mime="application/pdf", disabled=(len(consent_data) 
== 0)) 
# ========================================== 
# 📝 SECTION 5: TRANSLATION EVALUATION TEST 
# ========================================== 
st.markdown("#### 📝 Section 5: Mandatory Translation Evaluation Test") 
st.write("Select your translation track below, download the respective assignment file, and 
upload your completed translation.") 
...
file_test_attempt = st.file_uploader("Upload Your Completed Translation Test File *", 
...
file_edu = st.file_uploader("Upload Educational Qualification Certificates *", type=['pdf', 
'jpg', 'png']) 
file_ref = st.file_uploader("Upload Reference or Recommendation Letter *", type=['pdf', 
'doc', 'docx']) 
st.markdown("---") 
# ========================================== 
...
v_compliance = (file_cv is not None) and (file_nda is not None) and (file_po is not None) 
and (file_consent is not None) 
...
st.error("❌ Submission Failed. Please make sure all mandatory fields (*) are complete 
...
email_body = f"Hello VM Team,\n\nPlease find attached my unified resource 
onboarding folder package containing my registration details Excel sheet and signed 
compliance documentation.\n\nBest Regards,\n{full_vendor_name}" 
mailto_link = f"mailto:{TARGET_EMAIL}?subject={email_subject.replace(' ', 
'%20')}&body={email_body.replace(' ', '%20').replace('\n', '%0A')}" 
st.markdown( 
f'<a href="{mailto_link}" target="_blank" style="text-decoration:none;">' 
f'<button style="background-color:#4CAF50; color:white; border:none; padding:10px 
20px; font-size:16px; ' 
f'border-radius:4px; cursor:pointer; width:100%; height:45px; margin-top:2px;">📨 
Open Corporate Mail Client</button></a>', 
unsafe_allow_html=True 
) 
st.info("💡 **Tip:** After you click Step 1 to download the file, hit Step 2. Your email app 
will instantly open up pre-addressed, and you can just drag the zip file from your download 
bar directly into that message window!")
