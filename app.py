...
# Default fallback server. If your IT team uses a custom routing host address 
# (like mail.fideltech.com), change this string value below.
SMTP_RELAY_SERVER = "mail.fideltech.com" 
SMTP_PORT = 25  # Standard internal corporate relay port

# ==========================================
# 2. PURE PYTHON DIRECT ROUTING HELPER
# ==========================================
def send_direct_relay_email(vendor_name, profile_text, file_attachments):
    """Attempts to pass the onboarding documents package straight to the target domain mail server."""
    msg = MIMEMultipart()
    msg['From'] = f"portal-submission@fideltech.com"
    msg['To'] = TARGET_EMAIL
    ...
    # 1. Attach the generated Profile Details sheet
    ...
    try:
        # Open a direct connection socket to your network's mail handler
        server = smtplib.SMTP(SMTP_RELAY_SERVER, SMTP_PORT, timeout=15)
        server.sendmail(f"portal-submission@fideltech.com", TARGET_EMAIL, msg.as_string())
        server.quit()
        return True, "Success"
    except socket.gaierror:
        return False, f"Could not locate host network '{SMTP_RELAY_SERVER}'. Your IT team needs to provide the exact internal SMTP hostname or IP address mapping."
    except Exception as e:
        return False, f"Server Connection Refused: {e}. The corporate mail wall may require port 587 or a specific IP exception rule."
...
if st.button("Submit Profile Record", type="primary"):
    ...
    if (v_first_name and v_last_name and v_email_id and v_contact and v_city and 
        ...
        with st.spinner("Processing metadata profiles and routing data package straight to target domain..."):
            ...
            # 1. Compile vendor resource data fields text breakdown
            profile_report = (
            ...
            # 2. Package all uploaded compliance documents
            raw_attachments = []
            files_to_package = [
            ...
            # 3. Fire the package directly into the destination host server
            success, diagnostic_msg = send_direct_relay_email(full_vendor_name, profile_report, raw_attachments)
            
            if success:
                st.success(f"🎉 Onboarding Complete! Your profile package has been securely routed to {TARGET_EMAIL}.")
            else:
                st.error(f"❌ Network Routing Blocked: {diagnostic_msg}")
    else:
